...
ipcMain.handle('new-window', async (event, arg) => {
  ...
  const mainWindow = new BrowserWindow({
    ...
    y: windowBounds.y,
    webPreferences: {
      preload: path.resolve(app.getAppPath(), preLoadFiles[arg]),
...
ipcMain.handle('new-home-window', async (event) => {
  ...
  const mainWindow = new BrowserWindow({
    ...
    height: 700,
    webPreferences: {
      preload: path.join(__dirname, 'src', preLoadFiles['index.html']),
      contextIsolation: true
    }
  });
...
const createWindow = () => {
  ...
  const mainWindow = new BrowserWindow({
    ...
    height: 700,
    webPreferences: {
      preload: path.resolve(app.getAppPath(), 'src', preLoadFiles['index.html']),
      contextIsolation: true
    }
  });

...
function sendMessage(client, pose) {
    ...
    const p = pose[0];
    const pose3D = myStash.inputParams.map(x => p.annotations[x]);
    const pose2D = myStash.inputParams.map(x => p.annotations[x].map(y=> y.slice(0, 2)));

    const poseMessage = myStash.z ? pose3D : pose2D;
    const message = poseMessage.flat(2);
    window.oscApi.sendMessage(client.address, message);
}
